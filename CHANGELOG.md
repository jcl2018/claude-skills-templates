...
- **D000023: the nightly eval CI no longer exposes `ANTHROPIC_API_KEY` to non-`main` workflow runs or to a `bypassPermissions` model spawned from attacker-influenced eval fixtures.** Two HIGH-severity findings from the v2.0.7 `/ship` pre-landing review are closed together via one trust-boundary fix (Approach C). **F1 (secret-exfil ingress):** `.github/workflows/eval-nightly.yml` previously placed `ANTHROPIC_API_KEY` in a job-level `env:` while also exposing `workflow_dispatch`, so anyone with write access could `gh workflow run eval-nightly.yml --ref <their-branch>` and that branch's code ran with the live key in env — log-masking does not stop code the attacker controls. The workflow is now split into a secretless `eval-structure` job (runs on every ref/trigger; no GitHub Environment binding so it structurally cannot receive the secret; its first step is a permanent adversarial probe that fails CI loudly if any secret-shaped variable is ever visible on a non-`main` ref) and a `needs:`-gated `eval` job that runs the real API suite only when `github.ref == 'refs/heads/main'` AND is bound to a protected GitHub Environment (`eval-secrets`) whose deployment-branch rule restricts the secret to `main`. The Environment binding is the actual access control; the `if:` is the belt. The old unconditional job-level `env: ANTHROPIC_API_KEY` is deleted in the same commit, and the secret is now surfaced only step-scoped on the two steps that need it — there is no intermediate state where a non-`main` ref can read the key. A non-`main` `workflow_dispatch` now runs structure validation only (discovery + `bash -n` + the F3 plant-test, no API call). **F3 (prompt-injection RCE):** `tests/eval/lib/run-case.sh` spawns `claude` with `--permission-mode bypassPermissions` against prompts/fixtures from `tests/eval/<skill>/<case>/`, which become attacker-influenced the moment a malicious case is committed. The old code kept `ANTHROPIC_API_KEY` in env behind a fail-OPEN selective denylist (it `unset` a hand-listed set of other secrets but never touched `ANTHROPIC_API_KEY`, and silently missed every unenumerated secret-shaped var). It is replaced with a trust-gated scrub: the trusted scheduled-`main` path (cases are all merged/reviewed; F1's Environment+`main`-gating is the structural control) keeps env auth, while every other path (non-`main` CI and local dev) rebuilds the child env from an explicit allowlist via `env -i` and runs a fail-closed post-assert that aborts the case non-zero if any `^ANTHROPIC` / `*_API_KEY` / `*_TOKEN` / `*_SECRET` variable is present in the inherited env before the spawn. Local dev is unaffected (auth there is OAuth/keychain, not the env var). New `tests/eval/lib/plant-test.sh` is a zero-API-cost regression guard that plants `ANTHROPIC_API_KEY`, an unenumerated `FOO_API_KEY`, and a `*_TOKEN` variant and proves the assert fires before the model is ever spawned (a `claude` stub + sentinel file detects any fail-open regression), wired so `scripts/eval.sh` discovery skips it. Residual same-UID non-env recovery (files / `/proc` / open FDs) is explicitly out of V1 scope and tracked (Approach B). The autoplan CEO review surfaced SEC-1 (npm-postinstall key exposure on the scheduled-`main` job, same root cause one step upstream of F3); per the confirmed F1+F3-only scope, SEC-1 plus the pre-existing F11/F12 hardening items are split into dedicated active TODOS.md follow-up rows rather than expanding this fix.
