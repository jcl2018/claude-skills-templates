...
#   6. recap      — land/PR human-readable recap formatter (F000068 / S000112): a
...
#
# Args:
#   --phase {worktree|telemetry|pr-check|ship|cleanup|sync|recap}
...
case "$PHASE" in
  worktree|telemetry|pr-check|cleanup|sync|recap) ;;
  *)
    echo "[common-usage-phase] --phase required (one of: worktree, telemetry, pr-check, ship, cleanup, sync, recap)" >&2
    exit 1
    ;;
esac
