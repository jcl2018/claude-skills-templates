...
# Step 3k: the deterministic-only enrollment seam (topic_contracts_deterministic:)
# — the three-arm negative drill for the two-list topic contract. The three
# cj_goal verb topics (goal-feature / goal-task / goal-defect) are enrolled
# DETERMINISTIC-ONLY (three required points; agentic rows tolerated, never
# required), so three properties need non-vacuous proof, each planted
...
echo ""
echo "=== Step 3k: deterministic-only enrollment seam (3-arm negative drill, engine-only) ==="
