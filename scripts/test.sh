...
if [ -n "$(git status --porcelain 2>/dev/null)" ]; then
  ...
  # (c) Apply the documented override
  sed -i.bak 's/^last-updated:.*/last-updated: "'"$(date +%Y-%m-%d)"'"/' skills/CJ_system-health/USAGE.md && rm skills/CJ_system-health/USAGE.md.bak
