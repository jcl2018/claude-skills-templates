...
**Update (2026-05-16, PR #150 / D000021):** independently re-confirmed pre-existing — `scripts/test-deploy.sh` byte-identical to `origin/main`, fails identically with D000021 changes stashed. Root cause is local `~/.claude/` deployed-template drift from concurrent sessions (missing `~/.claude/templates/*` files, `installed 4.6.x ≠ current` skew), NOT a `test-deploy.sh` code bug. **CI-clean confirmed:** PR #150's `validate` check passed GREEN on a fresh runner — fresh CI has no global `~/.claude` drift, so Test 8 passes there. This validates the "Likely fix area" above: the durable fix is to isolate the doctor manifest in Test 8 so global state can't bleed in (the experimental-skill WARN is secondary). Local remedy meanwhile: `skills-deploy install` from the main checkout.
