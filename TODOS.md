...
### Harden `setup-hooks.sh` against blind hook clobber (P2, S)
Follow-up deferred at /CJ_goal_run GATE #2 during PR #150 (D000021, v4.6.5). PR #150 wired `setup-hooks.sh` into `setup.sh` so the bootstrap auto-installs the D000013 post-merge hook. Side effect (disclosed in the v4.6.5 CHANGELOG, deliberately out of #150's minimal scope): `setup-hooks.sh` does `cat > "$HOOK_DIR/pre-commit"` / `cat > "$HOOK_DIR/post-merge"` — a blind, no-backup clobber. Because `setup.sh`'s update branch runs on *every* re-invocation, any operator- or tooling-customized git hook (Husky, lefthook, local debug hooks) is silently destroyed on the next `setup.sh` run.
**Fix:** before overwriting, `grep -q` the existing `# Auto-installed by scripts/setup-hooks.sh` sentinel — if absent (hook is operator-owned), write `<hook>.bak` (preferred) or skip with a clear stderr warning. Also write via temp file + atomic `mv` so a mid-write failure leaves no corrupt/truncated hook (an adversarial reviewer flagged that the `|| echo >&2` guard in `setup.sh` swallows partial-write failures, leaving a broken hook present rather than absent). Small shell change to `scripts/setup-hooks.sh` + a regression assertion in `scripts/test.sh` near the D000013 block.
