...
### Phase 3 lifecycle-gate auto-update gap: /ship and /land-and-deploy don't update trackers (P2, M)
After PR #65 shipped + merged + deployed today, F000010_TRACKER.md and S000017_TRACKER.md still have all Phase 3 gates UNCHECKED — the workflow ran successfully but the trackers don't reflect it. This makes the user's stated workflow incomplete: the original "two manual steps" (design-doc → work-item, E2E testing) was actually three — manually updating Phase 3 gates after /ship and /land-and-deploy is the third. /implement-from-spec (S000018) and /qa-work-item (S000019) are designed to handle Phase 1→2 and Phase 2→3 transitions per their SPECs, but /ship and /land-and-deploy are upstream gstack skills with zero personal-workflow tracker awareness. **Options to evaluate:**
1. **Wrapper skills** (`/personal-workflow ship`, `/personal-workflow land`) that delegate to /ship + /land-and-deploy then update tracker gates. Pro: keeps gstack skills generic. Con: yet another skill to maintain; users might forget the wrapper exists and call /ship directly.
2. **Hook-based** (post-ship, post-deploy hooks update trackers based on git state). Pro: keeps invocation natural; works with /ship and /land-and-deploy unchanged. Con: requires new hook infra; harder to debug.
3. **Smart `/personal-workflow check --update`** that infers gate state from external signals (PR exists → /ship gate checked; PR merged → /land-and-deploy gate checked; CI green → smoke gate checked) and writes the inferred state back to trackers. Pro: leverages existing skill, idempotent. Con: separate manual command (you'd run after /ship, not automatic).
4. **Upstream contributions to gstack** — make /ship and /land-and-deploy emit tracker-mutation hooks. Pro: best long-term solution. Con: cross-repo work, slow feedback loop.
**Recommendation when revisiting:** option 3 (smart check --update) is the cheapest first cut. If it proves valuable, evolve toward option 2 (hooks) or option 4 (upstream).
**When:** before too many work items accumulate stale Phase 3 gates that need backfilling. Probably after /qa-work-item ships (S000019) so we know the full Phase 2→3 transition story. **Depends on:** S000019 shipped, several real work items run end-to-end. **Reference:** noticed during 2026-05-08 land-and-deploy of PR #65 (F000010).
...
### F000010 pipeline gap: implement+qa skills are user-story-only (P3, M)
`/scaffold-work-item` handles all 4 work-item types (feature/user-story/task/defect), but `/implement-from-spec` (S000018 SPEC AC-7) and `/qa-work-item` (S000019 SPEC AC-7) explicitly reject anything that's not a user-story. This means the pipeline can scaffold a defect tree (TRACKER + RCA + test-plan) but cannot implement or QA it through the F000010 chain — the user has to drop back to manual implementation for any defect, task, or feature-level work. The gap was discovered while planning the Phase 3 lifecycle-gate auto-update dogfood: the scaffold-work-item idempotency hole (P3/S) is naturally a defect, but force-fitting it as a user-story would require artificial DESIGN.md / SPEC.md scaffolding around a 30-line bug fix. **Options to evaluate:**
1. **Generalize implement/qa per-type** — both skills branch on the work-item type field and adapt. For defects, implement reads RCA + test-plan instead of SPEC + DESIGN; qa runs the test-plan instead of TEST-SPEC. Pro: pipeline becomes uniform. Con: each skill grows ~2-3x in size; type-specific code paths are harder to test.
2. **Add per-type sibling skills** — `/implement-from-rca`, `/qa-from-test-plan`, etc. Pro: each skill stays focused. Con: skill explosion (2 types × 2 skills = 4 new); user has to remember which to invoke for which type.
3. **Smart `/personal-workflow implement` and `/personal-workflow qa` dispatchers** — read the type field and delegate to the appropriate underlying skill. Pro: one entry point per phase regardless of type. Con: extra indirection layer.
4. **Accept the constraint** — defects/tasks/feature-level work continues to be implemented manually; only user-stories use the pipeline. Pro: zero new code. Con: pipeline coverage stays partial; user keeps switching workflows depending on type.
**Recommendation when revisiting:** option 1 (generalize per-type) is the cleanest long-term shape — fewer skill names to remember, and each skill already has the type-detection plumbing from /scaffold-work-item to model on. Option 4 is acceptable for the v1 quarter; revisit if defect/task work-items pile up faster than user-stories. **When:** after F000010's three skills have been used on real work items for 2-4 weeks and the pattern (or lack of it) for non-user-story work emerges. **Depends on:** organic demand from real work items. **Reference:** discovered 2026-05-08 while planning the Phase 3 lifecycle-gate auto-update dogfood.
