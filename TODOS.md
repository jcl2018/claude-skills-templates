...
### `/CJ_personal-pipeline` Step 7 strict halt-on-ambiguous blocks defects (P3, S)
`skills/CJ_personal-pipeline/pipeline.md` Step 7 (Phase 3 — QA Subagent) treats `RESULT: SMOKE=green; E2E=ambiguous; PHASE2_GATES=green` as halt-at-gate per the existing rule "Any red/ambiguous → User Challenge — Halt-at-Gate". But for `defect` and `task` work-item types, `/CJ_qa-work-item` structurally cannot return `E2E=green` — the inner E2E subagent only dispatches for user-stories. So `E2E=ambiguous` from a defect/task QA always means "n/a for this type", not "uncertain test result". Strictly applied, the pipeline can never reach `end_state=green` on a defect; on D000017 (PR #84), the orchestrator had to log a Taste decision treating ambiguous as green to proceed. **Fix:** make Step 7's halt-on-ambiguous type-aware. Logic: if `WORK_ITEM_TYPE in {defect, task}` and `E2E=ambiguous` and `SMOKE=green` and `PHASE2_GATES=green`, treat as green and continue silently (no taste decision needed). Step 8's tracker-journal parser already handles defect QA correctly via `[qa-pass]` entries, so the post-QA gate is unaffected. Read `WORK_ITEM_TYPE` from the tracker frontmatter at orchestrator side (it's already loaded when the orchestrator validates the work-item dir). **When:** before the next defect/task ships through `/CJ_personal-pipeline`. **Reference:** D000017 pipeline run 20260510-002554-10809 — full audit trail in `~/.gstack/analytics/CJ_personal-pipeline-auto-decisions.jsonl`.
