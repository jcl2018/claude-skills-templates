...
`CI-nightly` cadence pair. `/CJ_test_audit` gains the five structural checks
(a–e) via `test-spec.sh --check-structure` + idempotent doc-stub seeding
(`--seed-docs`), and `/CJ_test_run` gains
...
create an empty `tests/CI-nightly/`). It COEXISTS with the existing
