...
the agent-judged doc/test-drift audit), but F000080 REMOVED that workflow: the
audit (`scripts/audit-nightly.sh`, still driving `/CJ_doc_audit` + `/CJ_test_audit`)
now runs ON-DEMAND / locally, off the build path, filing findings to the
`audit-drift` GitHub issue — see `## Doc-sync coverage`. So `windows-nightly.yml`
is the only remaining `CI-nightly` workflow.
...
means "QA skips the inline agent-judged audit; it runs on-demand off the build
path"), and the audit instead runs ON-DEMAND / locally (`scripts/audit-nightly.sh`,
or the `/CJ_doc_audit` + `/CJ_test_audit` verbs directly), filing any findings to a
GitHub issue labelled `audit-drift`. (F000080 removed the former
`.github/workflows/audit-nightly.yml` nightly CI job — the script stays, the
schedule is gone.) Standalone `/CJ_qa-work-item` still runs the audit inline
...
deferred follow-ups.
The **category-based test contract** (F000074 foundation; **two-axis model by
F000078**) classifies every test on TWO orthogonal axes plus a mode. The test
**category** is the closed set `{workflow, regression, infra}` (`workflow` =
proves a whole workflow runs end-to-end, features earn these; `regression` =
proves a past defect stays fixed, defects earn these; `infra` = the standing
verification surface — the validator, the full suite, the deploy harness). The
verification **layer** is the closed set `{CI-push, CI-nightly, pipeline-gate,
local-hook}` (`CI-push` = green per-PR to ship; `CI-nightly` = the slower cadence
off the PR path; `pipeline-gate` = inline orchestrator halts; `local-hook` =
pre-commit + local-only harnesses). A per-test **mode** `{deterministic, agentic}`
completes it (`agentic` spends model tokens, so `agentic ⇒ tier ≠ free`);
`ratchet` is a per-test flag, not a layer. The category × layer pair threads the
physical surfaces: the folder a test lives in (`tests/<category>/<layer>/`), the
`categories:` overlay axis that declares it (each row carrying `category` +
`layer` + `mode`), the `docs/tests/<category>/<layer>/<name>.md` doc, the
`docs/tests/index.md` INDEX row, and the `/CJ_test_run` `--category` / `--layer` /
name arguments that run it. (Supersedes the F000075 taxonomy-V2 single axis
`{workflow, CI-push, CI-nightly}`, which conflated a kind with a cadence.) **The per-test doc is the authoritative What/How/Why
front door (F000077 — a GENERAL rule that now lives in `spec/test-spec.md`):**
each declared category test's `docs/tests/<category>/<layer>/<name>.md` is the ONE place a
...
`--category <workflow|regression|infra>` + `--layer <CI-push|CI-nightly|pipeline-gate|local-hook>` + single-name selection — all reusing
the `docs/tests/` name. `--check-structure` requires one `tests/<category>/<layer>/`
subfolder per DISTINCT declared (category, layer) pair that has ≥1 file-backed
test (so command-only `infra` rows never force an empty `tests/infra/…`); the new check **(f)** enforces
...
render (the physical migration of the 29 flat `tests/*.test.sh` into
`tests/<category>/<layer>/`, the feature→workflow/defect→regression enforcement
...
opt-in (the grandfather seam): **three topics are enrolled** — `portability`
...
local-hook). The 5 remaining labeled topics (deploy-harness — deliberately
unenrolled: its missing CI-push point is a conscious speed decision, and claiming
windows-smoke, portability's row, would double-count — plus cj-goal-eval,
doc-sync, e2e, cj-goal-gate) keep the advisory matrix until their missing
deterministic points are built (follow-up TODOs). Wired into `validate.sh` as
**Check 30** (HARD, registry-gated) + a targeted `scripts/test.sh` negative drill
proving BOTH directions (a removed deterministic point still FINDINGs + exits
non-zero; a removed agentic row exits 0 with the advisory note present — invoking
ONLY the engine, hermetic temp registries), surfaced in `/CJ_test_audit` Stage 1;
`/CJ_test_run` gains a `--topic`
...
overlay-amendment sweep DEFER to the on-demand audit (F000076 relocated the audit
off the inline path, F000079 the agentic sync; F000080 then re-homed both from a
nightly CI job to on-demand — see `## Deferred doc/test-drift audit`). Run manually
on demand, `/CJ_document-release` is still the
...
doc/test sync (prose rewrite + overlay-amendment sweep) DEFERS to the on-demand
audit (F000079; F000080 re-homed that audit off nightly CI):
...
- **Orchestrator paths** — each `cj_goal` orchestrator (`/CJ_goal_feature`,
  ...
  `/CJ_test_audit` sweep runs ON-DEMAND / locally instead — see `## Deferred
  doc/test-drift audit` below. The deterministic per-PR gate (`validate.sh` / `validate.yml` / the
...
## Deferred doc/test-drift audit (F000076; deferred SYNC safety net, F000079; re-homed off nightly CI to on-demand, F000080)
...
checkpoint AUQ. F000076 moved it OFF the per-PR path; F000080 then re-homed it
from a nightly CI cadence to ON-DEMAND / local (the nightly workflow was deleted,
the `scripts/audit-nightly.sh` script stays).
...
`DEFER_AUDIT: true`, which now means **"QA skips the inline agent-judged audit; it
runs on-demand off the build path."** **F000079 widened what this deferred sweep
is the safety net FOR:** the orchestrators also stopped running the slow inline
doc-sync (`/CJ_document-release` prose rewrite → replaced by a deterministic
doc-regen at Step 5.5) and the agent-judged test-sync overlay-amendment sweep (QA
8.6a/8.6b, now `DEFER_SYNC: true`). That deferred agentic drift is caught by the
SAME on-demand sweep — `/CJ_doc_audit` Stage 3 (docs reflect reality) +
`/CJ_test_audit` (overlay truthfulness) — filed to the same `audit-drift` issue.
The consciously-accepted trade: semantic doc/spec *prose* freshness now
...
- **Producer** — `scripts/audit-nightly.sh`, run ON-DEMAND / locally (the former
  `.github/workflows/audit-nightly.yml` nightly CI job was removed by F000080; the
  script also stays runnable directly, and `/CJ_doc_audit` + `/CJ_test_audit` can
  be invoked as verbs). It runs both audit verbs headless (`claude --print`),
...
doc/test audit moved off the inline path (it now runs on-demand — see `## Deferred
doc/test-drift audit`). Each
...
- **Check 30** — the **three-layer topic contract** (HARD, registry-gated,
  ...
  `full-suite` today) MUST carry ≥1 `CI-push` + ≥1 `CI-nightly` + ≥1
  ...
  **declaration-only ⇒ CI-safe, zero model spend** (an agentic BEHAVIOR, where a
  ...
  Paired with a targeted `scripts/test.sh` negative drill proving BOTH directions
  ...
  portability's agentic row yields exit 0, findings=0, and the advisory note).
...
`/CJ_document-release` (the standalone / on-demand doc-sync wrapper — no longer the
...
doc/test audit runs on-demand, off the build path, not per-drained-TODO;
`--quiet`'s other
