...
`doc/SKILL-CATALOG.md` (the consolidated catalog with ASCII workflow charts for
orchestrators and explicit `(single-step utility)` / `(validator)` /
`(phase-step in /CJ_goal_feature chain)` tags for everything else). Enforced
by `scripts/validate.sh` Check 15.
...
6. Add a section for the new skill to `doc/SKILL-CATALOG.md` using `templates/doc-SKILL-CATALOG-section.md` as a starting point. Include EITHER a fenced ASCII workflow chart (for orchestrators / phase-step skills) OR an explicit tag line `(single-step utility)` / `(validator)` / `(phase-step in /CJ_goal_feature chain)` (for validators and single-step utilities). Check 15 will ERROR if the section is missing OR if it has neither chart nor tag.
...
- `skill-catalog-completeness` — every active routable skill has a section AND every section names a real skill. Check 15b enforces.
