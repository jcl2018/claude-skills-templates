...
The `--dry-run` path prints `DRY RUN:` lines naming the planned worktree + the office-hours/scaffold/implement/qa/ship chain, then exits before any mutation. No writes happen. Determine the `end_state` it emits from the run itself.
