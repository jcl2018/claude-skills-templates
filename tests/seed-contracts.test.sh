...
#   (B2) CONSUMER with hand-authored overlay + NO
#       skills-catalog.json is NOT self → seeds   [D-fix regression]
...
if ! (
  ...
  # Second self-signal: a sandbox carrying a custom overlay AND a root
  # skills-catalog.json is also detected as self. The skills-catalog.json is the
  # workbench marker that gates the overlay signal (D-fix) — a consumer never ships
  # one, so the overlay-alone case (no catalog) is exercised in Case B2 below.
  S2=$(mk_sandbox)
  mkdir -p "$S2/spec"
  echo "overlay" > "$S2/spec/doc-spec-custom.md"
  echo "[]" > "$S2/skills-catalog.json"
  out2=$("$DEPLOY" seed-contracts --repo "$S2" 2>&1)
  printf '%s\n' "$out2" | grep -q 'workbench self-repo' || { fail_test "Case B: custom-overlay+catalog repo not detected as self (out: $out2)"; b_ok=no; }
  [ -f "$S2/spec/doc-spec.md" ] && { fail_test "Case B: doc-spec.md seeded into a canonical-contract repo"; b_ok=no; }
  [ "$b_ok" = yes ] && ok "Case B: workbench self-repo skipped (manifest-source match AND custom-overlay+catalog signal); contracts untouched"
  [ "$b_ok" = yes ]
); then ERRORS=$((ERRORS + 1)); fi

# ---------- Case B2: CONSUMER with a hand-authored overlay + NO catalog is NOT self ----------
# Regression guard (D-fix): is_workbench_self_repo's overlay signals (2)/(3) are
# gated behind a root skills-catalog.json. A real consumer that hand-authored a
# spec/doc-spec-custom.md (the documented way to declare repo-specific docs) and
# ships NO skills-catalog.json must be treated as NOT-self — so seeding + adoption
# proceeds, NOT skipped as "workbench self-repo". This is the live-confirmed bug:
# such a consumer's `skills-deploy install`/`seed-contracts` previously did nothing.
if ! (
  S=$(mk_sandbox)
  mkdir -p "$S/spec"
  # Hand-authored overlay WITHOUT the 'auto-generated by skills-deploy' marker, and
  # crucially NO skills-catalog.json at the repo root.
  echo "| Doc | Purpose | Requirement |" > "$S/spec/doc-spec-custom.md"
  out=$("$DEPLOY" seed-contracts --repo "$S" 2>&1)
  b2_ok=yes
  # MUST NOT be misdetected as the workbench self-repo.
  printf '%s\n' "$out" | grep -q 'workbench self-repo' && { fail_test "Case B2: consumer with hand-authored overlay + no catalog wrongly detected as self-repo (out: $out)"; b2_ok=no; }
  # Seeding MUST proceed: the three general contracts get seeded.
  printf '%s\n' "$out" | grep -q 'doc-spec: seeded' || { fail_test "Case B2: doc-spec not seeded for a consumer (adoption skipped — the bug; out: $out)"; b2_ok=no; }
  [ -f "$S/spec/doc-spec.md" ] || { fail_test "Case B2: spec/doc-spec.md not seeded into the consumer repo"; b2_ok=no; }
  # The hand-authored overlay is left untouched (seeding only ADDS the general file).
  [ -f "$S/spec/doc-spec-custom.md" ] || { fail_test "Case B2: hand-authored overlay was removed"; b2_ok=no; }
  [ "$b2_ok" = yes ] && ok "Case B2: consumer with hand-authored overlay + no skills-catalog.json treated as NOT-self — contracts seeded, adoption proceeds"
  [ "$b2_ok" = yes ]
); then ERRORS=$((ERRORS + 1)); fi
