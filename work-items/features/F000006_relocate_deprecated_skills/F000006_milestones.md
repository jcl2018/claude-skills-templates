...
| 2 | Working branch created (`feat/relocate-deprecated-skills`) | 2026-05-03 | Not Started | chjiang | Branch off main; populate `branch` field in trackers | #1 |
| 3 | S000013 implementation: file moves + catalog updates + skills-deploy / validate.sh / test.sh refactor + CLAUDE.md + deprecated/README.md + README regen | 2026-05-05 | Not Started | chjiang | All edits in one PR. Order: file moves → catalog → script refactors → tests/docs | #2 |
| 4 | T000014 verification: clean-target install with and without `--include-deprecated`; mirror invariant byte-check; doctor INFO at new path | 2026-05-05 | Not Started | chjiang | Test plan covers all 6 regression cases (see T000014_test-plan.md) | #3 |
| 5 | `/personal-workflow check` + `./scripts/validate.sh` + `./scripts/test.sh` clean on feature branch | 2026-05-05 | Not Started | chjiang | validate.sh Error check 10 PASS (7 mirror entries byte-identical); test.sh Failures: 0 | #4 |
