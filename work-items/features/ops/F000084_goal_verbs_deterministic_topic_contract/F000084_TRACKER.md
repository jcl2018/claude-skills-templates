...
- [decision] 2026-07-06 — RECONCILED onto F000086 (parallel land, v6.0.125) at merge time. F000086 shipped the SAME goal (deterministic-only enrollment) via a different, broader mechanism: it demoted the `local-hook`+`agentic` point to ADVISORY for ALL enrolled topics in the SINGLE `topic_contracts:` list. That superseded this feature's `topic_contracts_deterministic:` second-list engine seam (Part 1), which was DROPPED — the three goal verbs now enroll in the existing advisory `topic_contracts:` list (`[portability, validator, full-suite, goal-feature, goal-task, goal-defect]`). All the VALUE survived unchanged: the 4 new deterministic tests (defect smoke + 3 TEST_FAST-gated chain drills), the 9 per-verb categories rows, the re-topic'd eval rows (advisory), the 3 dream docs + 3 topic subdirs + front-door docs, and the 3-arm negative drill (retained as `Step 3k-topics`, verified against F000086's engine: CI-nightly removal fires, agentic-removal stays green, hidden dream doc fires). Post-merge validate.sh PASS (0 errors); Check 30/31 green at enrolled=6.
