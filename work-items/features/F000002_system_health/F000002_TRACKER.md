...
- 2026-04-24: Consolidation. Renamed F000002 from `system_health_v1` → `system_health` so each skill maps to exactly one feature. Status flipped from `closed` → `shipped` to align with the rest of the canonical-per-skill features. Future system-health work goes here regardless of version.
