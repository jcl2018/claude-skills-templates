...
- [x] **Pre-landing review hardening (3-subagent review at /ship Step 9):** Testing specialist flagged 2 CRITICAL coverage gaps — `exit $rc` propagation and the backup-fail abort branch had no static anchor, so a regression there would stay green and re-open the exact masked-failure / un-backed-clobber classes D000022 kills. Added 2 more D000022 `grep -qF` guards in the same `test.sh` D000013 block (`exit $rc`; the `could not be backed up — refusing to overwrite` abort string), negative-tested RED, plus a runtime cp-failure-shim scenario proving the abort path: ERROR emitted, exit 1, custom hook survives untouched, temp `rm -f`'d. Maintainability specialist: NO FINDINGS. Adversarial+security: 4 INFORMATIONAL, all non-blocking, "proceed to land"; the clobber-safety core (atomic mktemp+mv, sentinel gate, backup-before-overwrite, abort-on-backup-fail) confirmed sound; `.git/hooks` is already a local trust boundary so the sentinel-decoy vector is not a real threat.
...
- [ ] **Adversarial-review INVESTIGATE items (deferred to a separate follow-up — pre-existing/separate, explicitly non-blocking):** (1) `setup-hooks.sh` resolves `HOOK_DIR` from `--git-common-dir`/hooks and ignores `core.hooksPath` — for Husky/lefthook users git reads `.husky/`/`.lefthook/`, so the workbench hooks land in a dir git doesn't read and are silently inert (pre-existing blind spot, NOT introduced by D000022; causes inert hooks, not data loss). (2) A symlinked hook is flattened to a regular file by `mv` (content still recoverable in `.bak` + the symlink target survives). (3) `grep -qF` sentinel is a whole-file substring match — an operator hook that wraps/extends the generated hook and copies the sentinel header is mis-classified as ours and refreshed with no `.bak` (benign-extension footgun; the attacker framing is not real since `.git/hooks` is already a trust boundary). (4) `setup.sh`'s generic `WARN` is non-actionable — the backup-fail abort path is easy to miss in a long bootstrap. (5) no `trap 'rm -f "$tmp"' EXIT INT TERM` in `install_hook` — a kill between `mktemp` and the cleanup `rm -f` orphans a `.git/hooks/.<name>.XXXXXX` dotfile; git never executes it (wrong name + non-executable if killed pre-`chmod`) and mktemp's random suffix bounds growth, so it is pure cosmetic litter, not a failure/security class (surfaced by the /ship re-run adversarial pass; same non-blocking disposition as 1–4). None change D000022's clobber-safety core; expanding scope to fix them here contradicts the minimal-scope mandate. Flagged as a spawn-task follow-up.
...
- [finding] 2026-05-16 — Concurrent-merge handling at /ship Step 3: sibling PR #154 merged v4.6.7 to origin/main between the queue preflight and this ship; merged origin/main cleanly (my D000013-block test.sh edits and their F000025 test.sh additions are in non-overlapping regions — no conflict). Post-merge git hook fired and re-deployed via the NEW `install_hook` path (incidental dogfood). The pre-existing `test-deploy.sh` Test 8 manifest version-skew (D000021 journal) self-resolved post-merge (hook re-synced deployed manifest to 4.6.7 = VERSION); full `test.sh` now RESULT: PASS. New slot is v4.6.8 (v4.6.7 taken).
- [review] 2026-05-16 — /ship re-run (post Step 9.7): full checklist re-ran. validate.sh PASS; test.sh RESULT: PASS (all 5 D000022 guards + re-anchored D000013 guard + test-deploy.sh end-to-end green). 3-subagent re-review: Testing NO FINDINGS (both prior gaps confirmed closed), Maintainability NO FINDINGS (guards match source, shellcheck clean, comments accurate), Adversarial no new CRITICAL — recommends SHIP; surfaced one INFORMATIONAL 5th item (missing temp-cleanup `trap` → cosmetic dotfile litter on signal) folded into the same deferred follow-up. No fixes applied this invocation (deferral preserves minimal scope per the reviewer's own disposition) → Step 9.7 "no fixes → continue to Step 12".
- [review] 2026-05-16 — /ship Step 9: 3-subagent pre-landing review. Maintainability: NO FINDINGS. Adversarial+security: 4 INFORMATIONAL, all non-blocking ("proceed to land"); clobber-safety core confirmed sound; deferred to a separate follow-up (see Out of scope). Testing: 2 CRITICAL coverage gaps (exit $rc propagation; backup-fail abort branch) — AUTO-FIXED by adding 2 matching `grep -qF` static guards to the D000013 block in `test.sh` (repo's established CI-safe pattern, not a hermetic harness — D000021 precedent), negative-tested RED, plus a one-off runtime cp-shim scenario proving abort-without-clobber (ERROR, exit 1, custom hook survives, temp cleaned). The runtime evidence also corrected a flawed first injection (a single pre-created `.bak` obstacle is safely dodged by the timestamp fallback — correct behavior, not a bug). Per /ship Step 9.7, review fixes were applied → commit + STOP for a clean /ship re-run (re-test + version/CHANGELOG/push/PR).
