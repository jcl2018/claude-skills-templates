...
- 2026-07-04T00:00:00Z [land-blocker-fixed] Operator said "land it". CI `validate` was RED (windows-smoke PASS): a SIGPIPE in the reused-$_VALIDATE_OUT `printf|grep -q` early guards (S000094/S000096/F000060) — F000081's output growth crossed the 64KB pipe buffer, so `printf` took SIGPIPE and GitHub Actions' pipefail propagated it as a false `FAIL: … missing the swapped Check 24`. Fixed all 7 sites → pipe-free here-strings; verified rc=0 under pipefail with the real 145KB output, shellcheck clean. A pre-existing main bug (any large-output PR trips it). Re-pushed; awaiting green CI before land.
