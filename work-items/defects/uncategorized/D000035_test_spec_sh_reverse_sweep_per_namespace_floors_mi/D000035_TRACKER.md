...
- 2026-06-15 [qa-smoke] 1/2/3/5 (regression cases a/b/c/d): green — `bash tests/test-spec.test.sh` printed "PASS: test-spec", exit 0; covers test-plan rows 1-3 + 5 (consumer-surface no-misfire, present-but-zero-token namespace still-fires, rules-only unchanged, reserved-path-collision family-row gating no-false-fire).
- 2026-06-15 [qa-smoke] 4 (workbench coverage): green — `bash scripts/test-spec.sh --check-coverage` printed "OK coverage rows=69 reverse_tokens=49 findings=0", exit 0; no workbench regression on the strengthened fix.
- 2026-06-15 [qa-smoke-summary] green: 5/5 non-manual rows green (0 manual rows pending) on the strengthened fix.
- 2026-06-15 [qa-audit] AUDITS=deferred,spec_updates:test-spec-custom:none,doc-spec-custom:none (Step 8.6a/8.6b ran inline — strengthening touched existing scripts/test-spec.sh logic + added case (d) to the already-registered tests/test-spec.test.sh, no new test file/check/doc, no overlay row needed; 8.6c/8.6d DEFERRED via DEFER_AUDIT — orchestrator runs the post-sync audit).
- 2026-06-15 [qa-pass] D000035 (defect): green smoke from test-plan rows (5 rows). No qa-owned Phase 2 gates per template; Phase 3 `Test-plan verified` gate awaits /ship-time inference.
