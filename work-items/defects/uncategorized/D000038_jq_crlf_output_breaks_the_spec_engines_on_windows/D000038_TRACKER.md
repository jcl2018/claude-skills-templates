...
- 2026-07-01 [qa-smoke] R1 (T7 CRLF drill, re-run): green — `bash tests/workflow-spec-render.test.sh` RESULT: PASS rc=0 incl. both T7 OK lines (--list-orchestrators CR-free rc=0 under CRLF jq, no [workflow-spec-no-config]; --validate rc=0) — re-verified at commit 636570c (resume after docs-only audit remediation)
- 2026-07-01 [qa-smoke] R2 (live engine, re-run): green — `bash scripts/workflow-spec.sh --list-orchestrators` printed the 4 CJ_goal_* orchestrator names, rc=0 on the affected Windows machine (real CRLF jq first in PATH) at 636570c
- 2026-07-01 [qa-smoke] R3 (cascade repaired, cheap re-validation): green — re-ran for real: `bash scripts/test-spec.sh --validate` → OK schema_version=1 rc=0; `bash scripts/test-spec.sh --render-docs --check` → OK render, catalog in sync (findings=0) rc=0 (both prove Checks 24/26 stay green after the 636570c registry-row edit). Full-suite evidence CITED, not re-run: the full `bash scripts/validate.sh` PASS (Checks 24/26/27/28, Errors: 0) verified at fix commit 7a58439 by the prior QA run, PLUS the two subsequent commits (dcd337b, 636570c) each passed the pre-commit hook, which runs the full validate.sh — delta since the last green QA is docs/registry-only, code fix unchanged
- 2026-07-01 [qa-smoke-summary] green: 3/3 non-manual rows green (0 manual rows pending) — test-plan rows re-run as smoke-equivalent (defect type dispatch; resume re-dispatch) at commit 636570c
- 2026-07-01 [qa-audit] AUDITS=deferred,spec_updates:test-spec-custom:none,doc-spec-custom:none (Step 8.6a/8.6b ran inline — remediation commit 636570c already amended the test-workflow-spec-render row to name the T7 CRLF-jq drill; verified the overlay validates (`test-spec.sh --validate` OK) + rendered catalog fresh (`--render-docs --check` findings=0) + `doc-spec.sh --validate` OK; no rewrite, no new docs; 8.6c/8.6d DEFERRED via DEFER_AUDIT — orchestrator runs the post-sync audit)
- 2026-07-01 [qa-pass] D000038 (defect): green smoke from test-plan rows (3 rows, re-verified at 636570c after the audit-remediation commit). No qa-owned Phase 2 gates per template; Phase 3 `Test-plan verified` gate awaits /ship-time inference.
