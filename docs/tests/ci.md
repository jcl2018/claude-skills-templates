...
| windows-nightly workflow — nightly skills-deploy suite | ci | hard-fail | nightly manual | `.github/workflows/windows-nightly.yml` · `name: Windows Nightly (skills-deploy suite)` | Runs the full skills-deploy suite (test-deploy.sh) on windows-latest under Git Bash on a nightly schedule, with a manual dispatch trigger — the CI-nightly cadence windows-deploy test. |
