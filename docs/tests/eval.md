...
|-------|-------|-------------|---------|-----------------|---------|
| behavioral eval harness — headless skill evals | ci | hard-fail | nightly manual | `.github/workflows/eval-nightly.yml` · `scripts/eval.sh` | Spawns the headless CLI against scratch worktrees per eval case with JSON-schema output validation; budget-capped per case and per run. |
