# Test: `goal-feature-eval` (`workflow` / `local-hook`)
...
The /CJ_goal_feature workflow eval — drives the feature orchestrator through its dry-run chain-plan preview on the gstack-independent path (end_state dry_run_preview); backs the workflow-cj-goal-feature-runs level:workflow behavior; agentic, on-demand local-hook cadence.
...
(a headless model run spends tokens), so it lives at the `local-hook` layer — run
on-demand, never on a CI schedule or the free-tier default. The eval case is
