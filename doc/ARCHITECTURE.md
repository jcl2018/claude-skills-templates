...
**The surfacing — `/CJ_goal_feature` pipeline Step 4.6 (D4):** the verdict block would otherwise die in the wrapper RESULT — upstream `/ship` Step 18 regenerates the PR body from a fresh `/document-release` subagent that never sees it, and the F000030 subheadings only ever reached a PR via a human typing them. So the `/CJ_goal_feature` pipeline adds a deterministic post-`/ship` Step 4.6 that reads the scratch file and `gh pr edit <PR#>` to insert-or-replace the `### Registered-doc requirements` subsection under the PR body's `## Documentation` section. Best-effort (a failed `gh pr edit` logs a note and does NOT fail the run); NO upstream `/ship` modification. v1 wires surfacing into `/CJ_goal_feature` only (the PR-stop orchestrator where review matters most); `/CJ_goal_defect` + `/CJ_goal_todo_fix` surfacing is a Job-2.1 follow-up. The Step 6.7 producer is shared by all three.
