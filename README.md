...
| CJ_goal_investigate | Defect-to-shipped-fix orchestrator (v1.0 single-defect mode). Resolves a defect work-item by D-ID or fragment, dispatches /investigate as an Agent subagent with sentinel-wrapped JSON output, parses the response through a 9-state halt-on-red taxonomy, writes RCA + test-plan artifacts, and chains /CJ_qa-work-item → /ship → /land-and-deploy --suppress-readiness-gate. Iron-Law gate enforced for free; DONE_WITH_CONCERNS halts pre-ship at [investigate-unverified]. 5-row idempotency resume table; --dry-run preview. Workbench-only. Drain mode, --quiet, family-drain lock, sunset criterion, freestanding defect convention all deferred to v1.1+. | experimental | standalone | 1.0.0 |
| CJ_goal_auto | Full-handoff one-liner-to-deployed orchestrator (v1.0, experimental). Takes a single one-line idea and runs Stage 0 (worktree + version-queue + --handoff capability sentinel grep) -> Stage 0.5 (small-unambiguous classifier; halts non-small) -> Stage 1 (workbench-owned design-doc generator) -> Stage 1.5 (fail-closed post-condition doc gate) -> Stage 2 (/CJ_goal_run <doc> --handoff --no-drain). GATE #1 (autoplan final-approval AUQ) always human; GATE #2 (post-/ship merge gate) delegated to scripts/cj-handoff-gate.sh -- frozen merge-base, denylist (rename/symlink-safe + test-surface), <=120 added lines, <=5 files, Phase-2 markers all-green -- exit 0 proceeds to /land-and-deploy --suppress-readiness-gate; non-zero halts for human review of the created PR. Three explicit shapes: '<idea>' (human-gated default), --auto-merge-small-diffs '<idea>' (opt-in auto-merge), --dry-run '<idea>' (zero-write preview). --handoff is deprecated alias. Stage 3 writes per-run audit receipt to ~/.gstack/analytics/CJ_goal_auto.jsonl; --audit/--list-handoffs prints the last 10. Every-run retro AUQ for first 5 auto-merges, then every-5th. Halt-on-red default with structured stop block. Workbench-only (macOS). v1.0 single user-story; multi-story/headless-office-hours/Approach C/Copilot bundle all deferred. | experimental | standalone | 1.0.0 |
=======
| CJ_improve-queue | Workbench self-improvement skill. Three modes: (1) evaluate <url> — fetch an Anthropic best-practices article, classify pattern fit against existing workbench skills via subagent reasoning, append a draft TODOS.md row if novel/conflict. (2) audit — offline repo self-scan for stale skills + missing frontmatter; emits draft rows directly. (3) research <topic> — orchestrator-driven WebSearch + per-result evaluate, with privacy gate. All rows land with <!--impr-draft--> markers; /CJ_suggest skips them until promoted. Workbench-only (macOS); domain allowlist + HTML-comment-wrap defense; mkdir-based write lock; atomic mv; backup rotation. | experimental | standalone | 0.2.0 |
| CJ_goal_investigate | Defect-to-shipped-fix orchestrator (v1.0 single-defect mode). Resolves a defect work-item by D-ID or fragment, dispatches /investigate as an Agent subagent with sentinel-wrapped JSON output, parses the response through a 9-state halt-on-red taxonomy, writes RCA + test-plan artifacts, and chains /CJ_qa-work-item → /ship → /land-and-deploy --suppress-readiness-gate. Iron-Law gate enforced for free; DONE_WITH_CONCERNS halts pre-ship at [investigate-unverified]. 5-row idempotency resume table; --dry-run preview. Workbench-only. Drain mode, --quiet, family-drain lock, sunset criterion, freestanding defect convention all deferred to v1.1+. | experimental | standalone | 1.0.0 |
>>>>>>> origin/main
