...
| CJ_goal_investigate | DEPRECATED ALIAS (F000027 closure, sunsets v6.0.0) — superseded by /CJ_goal_defect once defect proved out (D000026 / v5.0.14 / PR #184). Thin alias prints a one-line deprecation banner then delegates to /CJ_goal_defect for non-D-id args. Bare D-id args (^D[0-9]{6}$) are rejected — forwarding would slug the D-id as a description and mint a new D-id. To ship a fix for an existing D-id, install the deprecated skill directly via skills-deploy install --include-deprecated. Removal at v6.0.0. | standalone | 5.0.15 |
