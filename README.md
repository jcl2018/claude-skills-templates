...
| CJ_suggest | Print a ranked top-5 of next-up work items from TODOS.md and tracker frontmatter. Internal phase-step skill rows (CJ_scaffold-work-item, CJ_implement-from-spec, CJ_qa-work-item, CJ_personal-pipeline, *-workflow validators) are filtered by default; pass --include-internal to surface them. Optional --for-skill / --limit flags pre-filter and extend the candidate window for downstream callers like /CJ_goal_todo_fix. | experimental | local-only | 1.2.0 |
...
|------|-------------|-------------|---------|
| CJ_goal_run | DEPRECATED ALIAS (F000027, sunsets v6.0.0) — superseded by the two-verb refactor. This thin alias prints a one-line deprecation banner then routes to /CJ_goal_feature. Use /CJ_goal_feature to build a feature end-to-end (topic → reviewable PR) or /CJ_goal_defect to fix a bug (description → shipped fix). Removal is deferred to v6.0.0; in-flight pipelines finish under the old skill via skills-deploy install --include-deprecated. | standalone | 5.0.6 |
| CJ_goal_auto | DEPRECATED ALIAS (F000027, sunsets v6.0.0) — superseded by the two-verb refactor. This thin alias prints a one-line deprecation banner then routes to /CJ_goal_feature. Use /CJ_goal_feature to build a feature end-to-end (topic → reviewable PR) or /CJ_goal_defect to fix a bug (description → shipped fix). Removal is deferred to v6.0.0; in-flight items finish under the old skill via skills-deploy install --include-deprecated. | standalone | 5.0.6 |
| cj_goal_feature | DEPRECATED ALIAS (casing-fix follow-up to F000027, sunsets v6.0.0) — superseded by /CJ_goal_feature. Thin alias prints a deprecation banner then routes to the uppercase canonical. | standalone | 5.0.12 |
| cj_goal_defect | DEPRECATED ALIAS (casing-fix follow-up to F000027, sunsets v6.0.0) — superseded by /CJ_goal_defect. Thin alias prints a deprecation banner then routes to the uppercase canonical. | standalone | 5.0.12 |
