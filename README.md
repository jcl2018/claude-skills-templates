...
| CJ_personal-pipeline | INTERNAL -- invoked by /CJ_run. Do not call directly. Single-keystroke orchestrator over the 3 CJ_personal-workflow pipeline skills (CJ_scaffold-work-item, CJ_implement-from-spec, CJ_qa-work-item). Dispatches each phase as a fresh-context Agent subagent with file-only handoff, runs independent inter-step quality gates, pre-collects AUQs at orchestrator (subagents have no AUQ tool). Auto-decides intermediate AUQs using 6 principles, classifies each decision as Mechanical / Taste / User Challenge, and surfaces Taste + User-Challenge-Approved decisions at one final approval gate (Step 8.5; mirrors /autoplan's single-mode contract). Halt-on-red default; idempotent; sunset criterion built in. | experimental | standalone | 1.0.0 |
| CJ_run | Unified pipeline entry point. v0.2.0 ships: (a) APPROVED /office-hours design doc -> full pipeline (autoplan -> scaffold -> impl -> QA -> PR -> deploy), (b) NO ARG -> Branch(g) scans current branch's work-items/ for in-progress user-stories (Phase 1 green + Phase 2 not started) and hands off to (c) work-item directory -> Branch(f) phase-detection PLACEHOLDER (prints next-step guidance and exits 0; full dispatch table is tracked under S000039, blocked on F000016/S000036). Replaces /CJ_ship-feature (renamed) and the public /CJ_personal-pipeline routing (now internal). Halt-on-red default; idempotent via each sub-skill's own re-entry path; sunset criterion at 6th invocation. | experimental | standalone | 0.2.0 |
