---
name: CJ_company-workflow-guide
description: "Doc-driven development workflow, scaffolding conventions, and installation guide for the CJ_company-workflow skill."
type: workflow
version: 1.0.0
---
...
- **Template** (`templates/CJ_company-workflow/*.md`) -- the structural skeleton
...
After generation, run `CJ_company-workflow validate` to ensure the docs meet the
...
Run `CJ_company-workflow validate` iteratively during this step. File mode catches
...
3. Run `CJ_company-workflow validate` on modified docs after updates
...
The `CJ_company-workflow validate` command has two modes:

### File Mode

```
CJ_company-workflow validate <file>
```

Checks a single tracker file against rules derived from the matching template
at runtime (`templates/CJ_company-workflow/tracker-{type}.md`):
...
```
CJ_company-workflow validate <dir>
```
...
# Or copy manually:
cp -r skills/CJ_company-workflow/ ~/.claude/skills/CJ_company-workflow/
cp -r templates/CJ_company-workflow/ ~/.claude/templates/CJ_company-workflow/
```

### What Gets Deployed

```
~/.claude/skills/CJ_company-workflow/
...
~/.claude/templates/CJ_company-workflow/
...
```
Level 1: $REPO_ROOT/skills/CJ_company-workflow/     (workbench)
Level 2: ~/.claude/skills/CJ_company-workflow/       (deployed)
```

Templates resolve the same way: `$REPO_ROOT/templates/CJ_company-workflow/` then
`~/.claude/templates/CJ_company-workflow/`.
...
Then run `/CJ_company-workflow knowledge-doctor` in this repo. You should see
...
Run `/CJ_company-workflow knowledge-doctor` first. It prints the state of every
...
- **One-shot disable:** `AI_KNOWLEDGE_DISABLE=1 /CJ_company-workflow ...` — bypasses
...
becomes a full prompt-injection surface on every `/CJ_company-workflow`
...
Run `/CJ_company-workflow knowledge-doctor` to see the exact state of every
