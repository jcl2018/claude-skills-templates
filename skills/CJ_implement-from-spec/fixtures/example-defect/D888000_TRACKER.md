...
<!-- Synthetic fixture for /CJ_implement-from-spec on a DEFECT work-item type.
     NOT a real defect — lives under skills/CJ_implement-from-spec/fixtures/,
...
- [ ] /CJ_implement-from-spec writes `output/fixed.txt` per RCA + test-plan
- [ ] Tracker journal records the implementation step
- [ ] RCA doc's `## Fix Description` section reflects the actual approach taken

## Log

- 2026-05-08: Created. Synthetic fixture for /CJ_implement-from-spec defect-path manual testing (S000021/F000012).
...
<!-- Populated by /CJ_implement-from-spec dogfood run. Canonical state: empty (modulo .gitkeep). -->
...
- **Phase 2 gate ownership:** for defects, `/CJ_implement-from-spec` marks `RCA doc updated` + `Todos section reflects remaining work`. The `Fix committed` gate stays UNCHECKED until a real git commit lands (the skill writes files; commits are user/`/ship`-owned).

## Journal

- 2026-05-08 [decision] Fixture lives at `skills/CJ_implement-from-spec/fixtures/example-defect/` (parallel to `example-user-story/`). Synthetic ID D888000 doesn't pollute the real work-items tree.
