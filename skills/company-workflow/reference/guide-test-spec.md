...
How to pre-populate TEST-SPEC.md from the template.
...
- These run without AI execution. They catch structural regressions.

**Tier 2 — E2E Tests** (real skill execution):
- For each P0 acceptance criterion: write the steps a real user would take
  (e.g., "run `the work-track create command --type feature`", "check $AI_CONTENT_DIR/work-items/")
...
- These require AI skill execution. They catch behavioral regressions.
- **E2E test skill pattern:** For features with complex behavior, create a companion
  test skill at the E2E test skill for the feature that automates
  E2E verification: creates fixtures with known defects, invokes the skill under
  test, and checks the output. Reference the test skill in the Tier 2 section.

**Rule of thumb:** if you can check it with `grep` or `jq`, it's Tier 1.
If it requires AI execution to produce the output, it's Tier 2.
