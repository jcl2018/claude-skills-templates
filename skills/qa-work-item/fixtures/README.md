# /qa-work-item — Fixtures

Per the v1 design (Issue 3.1A from /plan-eng-review), `/qa-work-item` ships
...
```bash
/qa-work-item skills/qa-work-item/fixtures/example-user-story/
```
...
`/scaffold-work-item` uses F000010 itself as its golden fixture (real
work item, dogfooded). `/qa-work-item` can't dogfood the same way: the
