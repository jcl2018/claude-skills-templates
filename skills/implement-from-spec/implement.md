...
Implement a personal-workflow work-item per its input artifacts: read handoff
...
Run `/personal-workflow check` on the work-item directory. Implementation work
...
2. **test-plan.md** — defines post-fix behavior (acts as de-facto SPEC for defects). Extract:
   ...
   - `## Verification Steps` — checklist used during QA (forward to /qa-work-item).
...
### Step 5.user-story
1. **Unresolved placeholders** in SPEC.md body: any `{[A-Z_]+}` or `{lowercase_with_underscores}` patterns — e.g., `{ITEM_NAME}`, `{component}`, `{TBD}`. These mean `/scaffold-work-item` left placeholders the author didn't fill in.
...
Do NOT touch the QA-owned gates (`Acceptance criteria verified met`, `Smoke tests pass`) — those are owned by `/qa-work-item` and will be marked when QA passes.
...
Tracker:  {TRACKER}
Next:
  /qa-work-item {WORK_ITEM_DIR}        # verify implementation; transitions Phase 2 (user-story) or records [qa-pass] (defect/task)
```
...
`/qa-work-item` marks the qa-owned subset (where present), and commit-driven
...
|---|---|---|
| `Acceptance criteria verified met` | /qa-work-item (qa.md Step 9) | Untouched by /implement-from-spec |
| `Smoke tests pass` | /qa-work-item (qa.md Step 9) | Untouched by /implement-from-spec |
| `Todos section reflects remaining work` | /implement-from-spec (Step 10) | Marked CHECKED on green |
| `Files section updated with changed files` | /implement-from-spec (Step 10) | Marked CHECKED on green |

### defect (`tracker-defect.md`)

| Gate | Owner | Boundary check role |
|---|---|---|
| `Fix committed` | user / `/ship` (commit gate) | Untouched by /implement-from-spec; marked when actual git commit lands |
| `RCA doc updated` | /implement-from-spec (Step 10) | Marked CHECKED on green |
| `Todos section reflects remaining work` | /implement-from-spec (Step 10) | Marked CHECKED on green |
...
`/personal-workflow check --update`'s post-merge inference).

### task (`tracker-task.md`)

| Gate | Owner | Boundary check role |
|---|---|---|
| `Core changes committed (>=1 commit SHA in Log)` | user / `/ship` (commit gate) | Untouched by /implement-from-spec |
| `Todos section reflects remaining work` | /implement-from-spec (Step 10) | Marked CHECKED on green |
| `Files section updated with changed files` | /implement-from-spec (Step 10) | Marked CHECKED on green |
...
The /implement-from-spec skill makes the work-item Phase-2-ready by completing
its work and marking its tracker-content gates per the type's template.
`/qa-work-item` then verifies the implementation (and marks the qa-owned gates
