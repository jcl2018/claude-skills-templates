...
silent leaf subagents (scaffold → implement → QA, with the inline audits SKIPPED —
the orchestrator passes a literal `DEFER_AUDIT: true` directive so QA skips its
...
`already_shipped` with `next_action=` / `resume_cmd=`
