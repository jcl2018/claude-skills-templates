...
Step 3.4 (the QA-audit checkpoint, fed by the post-sync audit), then Step 4
(/ship).
...
audit), then Step 3.4 (the QA-audit checkpoint), then Step 4 (/ship).
...
Surface an AskUserQuestion **ALWAYS** — findings
