...
writes inline but SKIP the 8.6c/8.6d three-stage audit (the nightly CI audit job
covers it; it is NOT re-run inline). Return the RESULT line verbatim —
...
in the same PR as the code. (The agent-judged doc/test audit that used to run
after doc-sync now runs nightly in CI — F000076 — so the build path ends at
doc-sync → /ship, with no inline audit or checkpoint.)
