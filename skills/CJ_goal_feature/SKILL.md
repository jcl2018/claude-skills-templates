...
| `halted_at_qa_audit` | `[qa-audit-declined]` | Step 3.4 QA-audit checkpoint: operator chose Halt on the Step 8.6 AUDIT_FINDINGS digest (doc/test audits + spec-overlay updates). Continue past findings journals `[qa-audit-waived]` instead. The checkpoint fires ALWAYS (green or findings) — the one AUQ past the design gate. |
