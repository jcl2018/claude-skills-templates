# /CJ_personal-workflow check — Work Item Validation

Validate work items against the templates in `templates/CJ_personal-workflow/` and
...
Two tiers: Tier 1 (single file or directory, matches CJ_company-workflow validate
...
# TIER 1: Foundation
# Matches CJ_company-workflow validate approach: template + manifest-based checks.
...
2. `~/.claude/templates/CJ_personal-workflow/{template_filename}`
...
`Error: template tracker-{type}.md not found at {_TMPL_DIR} or ~/.claude/templates/CJ_personal-workflow/. Run skills-deploy install.` and stop.
...
3. Verify the TRACKER.md instance matches all four:
   ...
   - All required phases present (Track, Implement, Ship for CJ_personal-workflow)
...
4. **Never auto-marks `/CJ_personal-workflow check — validation passed`** in v1 — would risk recursion when called from this same skill. Documented gap; user can verify by running `/CJ_personal-workflow check` (no `--update`) separately.
...
# TIER 2: Extensions (CJ_personal-workflow only)
# These steps have no equivalent in CJ_company-workflow. They require walking the
...
- **Not a git repo:** "Error: /CJ_personal-workflow requires a git repository."
- **Skill assets not found:** "Error: CJ_personal-workflow skill assets not found."
...
- **Template not found (file mode):** "Error: template tracker-{type}.md not found at {_TMPL_DIR} or ~/.claude/templates/CJ_personal-workflow/. Run skills-deploy install."
