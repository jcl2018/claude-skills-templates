...
description: "QA a CJ_personal-workflow work-item against its test rows. User-stories get smoke tests + a fresh-context E2E subagent per TEST-SPEC; defects and tasks run their test-plan rows as smoke-equivalent. Every green path then runs the Step 8.6 audit block — refresh spec/test-spec-custom.md (8.6a), refresh spec/doc-spec-custom.md (8.6b), run /CJ_doc_audit inline (8.6c), run /CJ_test_audit inline (8.6d) — and audit findings ride the GREEN RESULT's AUDITS= field + a fenced AUDIT_FINDINGS block (the cj_goal post-QA checkpoint owns the Continue/Halt decision; findings never flip QA red). Writes findings to tracker journal, transitions Phase 2 QA-owned gates, refuses on incomplete Phase 2. Idempotent. Use when: 'QA this work-item', 'run tests on the work-item', 'verify the work-item'."
version: 1.0.0
allowed-tools:
  - Bash
  - Read
...
- **Audit block (Step 8.6, all types, every green path):** refreshes the two custom spec overlays FIRST (8.6a `spec/test-spec-custom.md` units for any test surface this work-item added/changed; 8.6b `spec/doc-spec-custom.md` rows for any repo-specific doc), THEN runs `/CJ_doc_audit` (8.6c) + `/CJ_test_audit` (8.6d) INLINE (a leaf subagent cannot spawn subagents — the QA agent executes the audit skills' SKILL.md logic directly). Audit findings ride the GREEN RESULT's `AUDITS=doc:<ok|findings:n>,test:<ok|findings:n>,spec_updates:<summary>` field + a fenced `AUDIT_FINDINGS` block — they do NOT flip QA red; the calling cj_goal orchestrator's post-QA checkpoint AUQ owns the Continue (`[qa-audit-waived]` on findings) / Halt (`[qa-audit-declined]`) decision.
