...
- **Audit block (Step 8.6, all types, every green path):** refreshes the two custom spec overlays FIRST (8.6a `spec/test-spec-custom.md` units for any test surface this work-item added/changed; 8.6b `spec/doc-spec-custom.md` rows for any repo-specific doc) — these WRITES always run inline. The three-stage audits (8.6c `/CJ_doc_audit` + 8.6d `/CJ_test_audit`) are **deferrable**: Step 8.6.0 inspects the dispatch prompt for the literal directive `DEFER_AUDIT: true` (the orchestrator-to-QA defer signal — NOT an argv flag, since this skill is dispatched as a subagent prompt).
  - **Standalone (no `DEFER_AUDIT: true`):** 8.6c/8.6d run INLINE (a leaf subagent cannot spawn subagents — the QA agent executes the audit skills' SKILL.md logic directly). Findings ride the GREEN RESULT's `AUDITS=doc:<ok|findings:n>,test:<ok|findings:n>,spec_updates:<summary>` field + a fenced `AUDIT_FINDINGS` block — they do NOT flip QA red; on a standalone run the operator reviews them (there is no build-path audit checkpoint — the orchestrators' agent-judged audit runs on-demand off the build path).
  - **Orchestrator-driven (`DEFER_AUDIT: true` in the dispatch prompt):** 8.6c/8.6d are SKIPPED; the RESULT reads `AUDITS=deferred,spec_updates:<summary>` and emits NO `AUDIT_FINDINGS` block. The agent-judged three-stage audit runs on-demand (locally via `/CJ_doc_audit` + `/CJ_test_audit`, or `bash scripts/audit-nightly.sh`), off the build path (F000076).
