---
name: CJ_run
description: "Unified pipeline entry point. Accepts (a) an APPROVED /office-hours design doc → full pipeline (autoplan → scaffold → impl → QA → PR → deploy), (b) a work-item directory → detect phase from TRACKER state and dispatch the right sub-pipeline (impl_qa_ship / qa_ship / ship / open_pr / already_shipped), or (c) no argument → scan current branch's work-items/ for in-progress user-stories and auto-resume. Replaces /CJ_ship-feature (renamed) and the public /CJ_personal-pipeline routing (now internal). Halt-on-red default; idempotent via each sub-skill's own re-entry path; sunset criterion at 6th invocation."
version: 0.2.0
allowed-tools:
  - Bash
  - Read
...
If `NOT_A_GIT_REPO`: tell the user "Error: /CJ_run requires a git repository." and stop.
...
if [ -n "$_REPO_ROOT" ] && [ -f "$_REPO_ROOT/skills/CJ_run/run.md" ]; then
  _SKILL_DIR="$_REPO_ROOT/skills/CJ_run"
fi
if [ -z "$_SKILL_DIR" ] && [ -f "$HOME/.claude/skills/CJ_run/run.md" ]; then
  _SKILL_DIR="$HOME/.claude/skills/CJ_run"
fi
...
if [ -z "$_SKILL_DIR" ]; then
  echo "ERROR: CJ_run skill assets not found."
...
```
/CJ_run <design-doc-path>   → full pipeline (Branches a/b/c/d)
/CJ_run <work-item-dir>     → phase-detect + dispatch (Branch f)
/CJ_run                     → scan branch's work-items/ + auto-resume (Branch g)
```
...
to `~/.gstack/analytics/CJ_run.jsonl` with `{run_id, design_doc, work_item,
...
```
/CJ_run <design-doc-path>      # Branches (a/b/c/d)
/CJ_run <work-item-dir>        # Branch (f)
/CJ_run                        # Branch (g)
```
...
/CJ_run ~/.gstack/projects/jcl2018-knowledge-base/chjiang-claude-stupefied-ellis-2949b6-design-20260511-220642.md

# Work-item-dir input: resume from current phase
/CJ_run work-items/features/ops/F000017_cj_run_entry_point/S000038_rename_and_branch_g
...
|---|---|---|
| Not a git repo | "Error: /CJ_run requires a git repository." | Run inside a repo |
| Skill assets not found | "Error: CJ_run skill assets not found." | Run `skills-deploy install` |
...
| Multi-story feature scaffold-only | "Multi-story feature scaffolded. Per-child invocation needed: ..." + per-child instructions | Invoke /CJ_run on each child design doc |
...
thereafter), the orchestrator reads `~/.gstack/analytics/CJ_run.jsonl`
...
To delete: remove `skills/CJ_run/`, strike the catalog entry,
