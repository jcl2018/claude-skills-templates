...
If `NOT_A_GIT_REPO`: tell the user "Error: /CJ_personal-pipeline requires a git repository." and stop.
...
the 3 underlying pipeline skills (CJ_scaffold-work-item, CJ_implement-from-spec,
CJ_qa-work-item) plus CJ_personal-workflow.

```bash
_REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
_SKILL_DIR=""

if [ -n "$_REPO_ROOT" ] && [ -f "$_REPO_ROOT/skills/CJ_personal-pipeline/pipeline.md" ]; then
  _SKILL_DIR="$_REPO_ROOT/skills/CJ_personal-pipeline"
fi
if [ -z "$_SKILL_DIR" ] && [ -f "$HOME/.claude/skills/CJ_personal-pipeline/pipeline.md" ]; then
  _SKILL_DIR="$HOME/.claude/skills/CJ_personal-pipeline"
fi

# Verify upstream skills exist (we dispatch to them via Agent tool)
for _UPSTREAM in CJ_scaffold-work-item CJ_implement-from-spec CJ_qa-work-item CJ_personal-workflow; do
...
if [ -z "$_SKILL_DIR" ]; then
  echo "ERROR: CJ_personal-pipeline skill assets not found."
...
in `~/.gstack/projects/{slug}/...-design-*.md` — and runs the CJ_personal-workflow
...
# Used by /CJ_goal_run Branch (b)/(f) to dispatch per-child runs without a design doc.
...
with `--work-item-dir` by wrapper skills like `/CJ_goal_run`) suppresses Step 8.5 and
...
|---|---|---|
| Not a git repo | "Error: /CJ_personal-pipeline requires a git repository." | Run inside a repo |
| Skill assets not found | "Error: CJ_personal-pipeline skill assets not found." | Run `skills-deploy install` |
...
| Boundary check at end fails | "Halt: post-{phase} gate found compliance break. See /CJ_personal-workflow check output." | Inspect, fix, re-invoke |
...
To delete: remove `skills/CJ_personal-pipeline/`, strike the `skills-catalog.json`
