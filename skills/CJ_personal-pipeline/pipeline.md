...
# written). Used by /CJ_goal_run and any future wrapper consuming this
# pipeline as a subagent. See "Suppression Contract" below.
# --work-item-dir <path> is the alternate input mode: skip scaffold, run
# impl+QA on a pre-staged work-item dir. Used by /CJ_goal_run Branch (b)/(f)
...
`/CJ_goal_run` that pre-scaffolded children). Skip footer search and Phase 1
...
The wrapper that set the flag (e.g. `/CJ_goal_run`) is consuming
...
When `/CJ_personal-pipeline` is invoked with `--suppress-final-gate` (typically by a wrapper skill like `/CJ_goal_run` that runs the pipeline as an Agent subagent and consumes the decision log itself):
