# /scaffold-work-item — Fixtures

Per the v1 design (Issue 3.1A from /plan-eng-review), `/scaffold-work-item` ships
with **one golden fixture** for manual snapshot-diff testing.

## Golden fixture

The canonical fixture is **F000010 itself**. F000010 (`work-items/features/personal-workflow/F000010_pipeline_skills/`)
...
- **Expected output:** `work-items/features/personal-workflow/F000010_pipeline_skills/`

## Manual snapshot-diff workflow

Once `/scaffold-work-item` ships, the bootstrap proof is:

```bash
# 1. Backup the hand-scaffolded baseline
cp -r work-items/features/personal-workflow/F000010_pipeline_skills/ /tmp/F000010-baseline/

# 2. Remove the existing F000010 work-item dir
rm -rf work-items/features/personal-workflow/F000010_pipeline_skills/
...
diff -r work-items/features/personal-workflow/F000010_pipeline_skills/ /tmp/F000010-baseline/
```
...
- Boundary check (`/personal-workflow check`) on the produced dir reports MISSING/DRIFT
...
baseline can be re-validated with `/personal-workflow check` and the fixture pointer here
stays correct.
