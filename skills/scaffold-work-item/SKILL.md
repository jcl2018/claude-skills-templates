---
name: scaffold-work-item
description: "Scaffold a personal-workflow work item from an /office-hours design doc. Reads design + templates + manifest + WORKFLOW.md, produces a compliant work-item directory tree with all required artifacts. Runs /personal-workflow check at boundaries; idempotent (re-run on same input is NO-OP)."
version: 0.1.0
allowed-tools:
  - Bash
  - Read
...
_TMPL_DIR=""         # personal-workflow templates

# Level 1: workbench repo
if [ -n "$_REPO_ROOT" ] && [ -f "$_REPO_ROOT/skills/scaffold-work-item/scaffold.md" ]; then
  _SKILL_DIR="$_REPO_ROOT/skills/scaffold-work-item"
fi
...
if [ -z "$_SKILL_DIR" ] && [ -f "$HOME/.claude/skills/scaffold-work-item/scaffold.md" ]; then
  _SKILL_DIR="$HOME/.claude/skills/scaffold-work-item"
fi
...
if [ -z "$_SKILL_DIR" ] || [ -z "$_PW_SKILL_DIR" ]; then
  ...
  echo "Need: scaffold-work-item AND personal-workflow assets."
...
If `NOT_FOUND`: tell the user "Error: scaffold-work-item or personal-workflow skill assets not found. Run `skills-deploy install` or check the repo structure." and stop.
...
- `personal-artifact-manifests.json` from `personal-workflow` (per-type artifact list)
- `WORKFLOW.md` from `personal-workflow` (hierarchy + scaffolding rules)
- Templates from `templates/personal-workflow/` (artifact structure)
...
It runs `/personal-workflow check` at start (gate input drift) and end (gate
...
```
/scaffold-work-item <design-doc-path>                   # type derived from branch
/scaffold-work-item <design-doc-path> --type {feature|user-story|task|defect}
```
...
|---|---|---|
| Not a git repo | "Error: /scaffold-work-item requires a git repository." | Run inside a repo |
| Skill assets not found | "Error: scaffold-work-item or personal-workflow skill assets not found." | Run `skills-deploy install` or check repo structure |
...
| Manifest missing | "Error: personal-artifact-manifests.json not found." | Reinstall personal-workflow skill |
| Template not found | "Error: template {filename} not found at {_TMPL_DIR} or ~/.claude/templates/personal-workflow/. Run skills-deploy install." | Run `skills-deploy install` |
...
| Boundary check at start fails | "Error: existing state in work-items/ has drift; refusing to scaffold on top. See /personal-workflow check output above." | Resolve drift, then re-run |
| Boundary check at end fails | "Error: scaffolded directory failed /personal-workflow check. See output above." | Inspect, fix the violation, or report bug |
